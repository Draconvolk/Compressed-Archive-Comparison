...
{
	public class WorkflowActions(CompressionResolver resolver, IInfo info) : IWorkflowActions
	{
		...
		public CompressionResolver CompResolver { get; } = resolver;
		...
		public async Task DisplaySource()
			=> await PrintFileList(CompResolver, SourceList, ConfigInfo);
		...
		public void IdentifyMissingFiles()
			=> MissingFiles = DataProcessing.GetMissingSourceFiles(CompResolver, SourceList, DestinationList);
		...
		public async Task DisplayMissingFiles()
			=> await PrintFileList(CompResolver, MissingFiles, ConfigInfo, "missing");
		...
		/// <returns></returns>
		public static async Task PrintFileList(CompressionResolver resolver, IEnumerable<string> fileList, IInfo info, string type = source)
		{
			await Task.Run(() =>
			{
				...
				try
				{
						...
						var output = new List<string>
					{
						...
						{
							var fileContents = DataProcessing.GetCompressedFileContent(resolver, file);

...
namespace CompressedArchiveComparisonTests
{
    ...
    public class ExportTests
    {
        ...
        {
            var result = await DProcessing.WriteToFile(TestData.FullPathMissingList, TestData.NormalizedEmptyFileName);
        ...
        {
            var result = await DProcessing.WriteToFile([], TestData.NormalizedEmptyFileName);
        ...
        {
            var result = await DProcessing.WriteToFile(TestData.FullPathMissingList, testData);
            var expectedResult = "The Following files were missing from the destination:"
                                 + Environment.NewLine
                                 + TestData.FullPathMissingList.OrderBy(x => x).FlattenToString(Environment.NewLine)
                                 + Environment.NewLine;
